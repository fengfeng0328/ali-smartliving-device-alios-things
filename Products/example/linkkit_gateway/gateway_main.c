...
int gateway_main_send_msg(gateway_msg_t *msg, int len)
{
...
int get_one_device_mate(int devid, iotx_linkkit_dev_meta_info_t *p_subdev)
{
    ...
    aos_kv_get(key_buf, p_subdev->product_key, &len);
    ...
    aos_kv_get(key_buf, p_subdev->product_secret, &len);
    ...
    aos_kv_get(key_buf, p_subdev->device_name, &len);
    ...
    aos_kv_get(key_buf, p_subdev->device_secret, &len);

    gateway_info("DevID(%d) Meta:", devid);
    gateway_info("PK=%s.", p_subdev->product_key);
    gateway_info("PS=%s.", p_subdev->product_secret);
    gateway_info("DN=%s.", p_subdev->device_name);
    gateway_info("DS=%s.", p_subdev->device_secret);
...
//Just for testing
static int handle_queue_msg(int master_devid, int timeout)
{
    int ret = 0;
    int recv_size = 0;
    gateway_msg_t msg;
    ...
    if (ret == 0 && recv_size == QUEUE_MSG_SIZE)
    {
        ...
        case GATEWAY_MSG_TYPE_QUERY_SUBDEV_ID:
        {
            ...
            memset(&subdev_mate, 0, sizeof(subdev_mate));
            get_one_device_mate(msg.devid, &subdev_mate);
            ret = gateway_query_subdev_id(&subdev_mate);
        }
        break;
        default:
...
int linkkit_main(void *paras)
{
    ...
    while (1)
    {
        ...
        if (gateway_ctx->cloud_connected)
        {
            handle_queue_msg(gateway_ctx->master_devid, GATEWAY_YIELD_TIMEOUT_MS / 2);
        }
