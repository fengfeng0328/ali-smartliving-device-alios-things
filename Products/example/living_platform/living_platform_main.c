...
#define ENABLE_REPORT_TEST	(0)

#if ENABLE_REPORT_TEST
...
int linkkit_main(void *paras)
{
	...
	if((0 == strlen(master_meta_info.product_key))||(0 == strlen(master_meta_info.device_name))\
		||(0 == strlen(master_meta_info.product_secret)))
	{
    ...
    if (0 == dynamic_register) {
        if (0 == strlen(master_meta_info.device_secret)) {
            EXAMPLE_TRACE("No device secret info found...\n");
            while (1) {
                aos_msleep(USER_EXAMPLE_YIELD_TIMEOUT_MS);
            }
        }
