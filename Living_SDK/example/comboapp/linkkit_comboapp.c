...
// for demo only
#define PRODUCT_ID     0
#define DEVICE_SECRET  "DS_NOT_SET"
#define DEVICE_NAME    "DN_NOT_SET"
#define PRODUCT_KEY    "PK_NOT_SET"
#define PRODUCT_SECRET "PS_NOT_SET"
...
extern char g_combo_pk[PRODUCT_KEY_LEN + 1];
extern char g_combo_ps[PRODUCT_SECRET_LEN + 1];
extern char g_combo_dn[DEVICE_NAME_LEN + 1];
extern char g_combo_ds[DEVICE_SECRET_LEN + 1];
extern uint32_t g_combo_pid;
...
void set_iotx_info()
{
    ...
    memset(g_combo_pk, 0, sizeof(g_combo_pk));
    memset(g_combo_ps, 0, sizeof(g_combo_ps));
    memset(g_combo_dn, 0, sizeof(g_combo_dn));
    memset(g_combo_ds, 0, sizeof(g_combo_ds));
    g_combo_pid = 0;

    len = PRODUCT_KEY_LEN+1;
    aos_kv_get("linkkit_product_key", g_combo_pk, &len);

    len = PRODUCT_SECRET_LEN+1;
    aos_kv_get("linkkit_product_secret", g_combo_ps, &len);

    len = DEVICE_NAME_LEN+1;
    aos_kv_get("linkkit_device_name", g_combo_dn, &len);

    len = DEVICE_SECRET_LEN+1;
    aos_kv_get("linkkit_device_secret", g_combo_ds, &len);

    len = sizeof(g_combo_pid);
    aos_kv_get("linkkit_product_id", &g_combo_pid, &len);
