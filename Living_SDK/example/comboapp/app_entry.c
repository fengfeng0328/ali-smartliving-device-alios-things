...
extern int combo_net_deinit(void);
...
void do_awss()
{
    aos_task_new("ble_awss_close", ble_awss_close, NULL, 2048);
...
{
    aos_schedule_call(do_ble_awss, NULL);
}
...
static void handle_linkkey_cmd(char *pwbuf, int blen, int argc, char **argv)
{
    if (argc == 1)
    {
        ...
        aos_kv_get("linkkit_product_id", &product_id, &len);
    ...
    else if (argc == 6)
    {
        ...
        product_id = (uint32_t)strtoul(argv[5], NULL, 0);
        aos_kv_set("linkkit_product_id", &product_id, sizeof(product_id), 1);
...
static struct cli_command awss_ble_cmd = { .name     = "ble_awss",
                                              .help     = "ble_awss [start]",
